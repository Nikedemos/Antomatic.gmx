///toggleCellAtScreen(screen_x,screen_y)

var maus_x=argument[0];
var maus_y=argument[1];

screenToGridRelative(maus_x,maus_y); //we get the return values in temp values

//now make sure it's the right chunk, so no matter what instance calls this function, it will work

with (global.temp[2])
//temp holds the called chunk. It will always be the right chunk at the right spot.
{
screenToGridRelative(maus_x,maus_y); //we get the return values in temp values
...
var everyone_value=analyseNeighbourTunnelsMoore(maus_x,maus_y);
