...
with (objWindow)
    ...
    //this only applies when the window has a bar
    if (global.wmx_curr>=window_x && global.wmy_curr>=window_y && global.wmx_curr<window_x+window_width && global.wmy_curr<window_y+window_height+global.window_bar_height)
        {
        //yes, there's a window underneath the window_mouse_x/y
        
        if (depth<min_depth_in_range)
            {
